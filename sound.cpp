...
/*------------------------------------------------------------------------------
   サウンドファイルの読み込み
------------------------------------------------------------------------------*/
int LoadSound(const char* pFilename_sound)
{
	HANDLE hFile;
	DWORD dwChunkSize = 0;
	...
	{
		//サウンド名を比較
		if (strcmp(g_SoundName[i], pFilename_sound) == 0)
		{
			return i;
		}
	}
	...
	// サウンドデータファイルの生成
	std::string str(pFilename_sound);
	...
	//読み込んだサウンド名を保存する
	strcpy_s(g_SoundName[g_SoundIndex], 256, pFilename_sound);
